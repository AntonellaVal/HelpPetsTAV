...
export class BdServicioService {
  ...
  //tabla rol
  tablaRol: string =
    'CREATE TABLE IF NOT EXISTS rol(id_rol INTEGER PRIMARY KEY autoincrement, nombre_rol VARCHAR(20) NOT NULL);';

  //tabla especie
  tablaEspecie: string =
    'CREATE TABLE IF NOT EXISTS especie(id_especie INTEGER PRIMARY KEY autoincrement, nombre_especie VARCHAR(20) NOT NULL);';

  //tabla usuario
  tablaUsuarios: string =
    'CREATE TABLE IF NOT EXISTS usuarios(id_usuario INTEGER PRIMARY KEY autoincrement, nombre_usuario VARCHAR(25) NOT NULL, apellido_usuario VARCHAR(25) NOT NULL, correo VARCHAR(50) NOT NULL,clave VARCHAR(16) NOT NULL, fecha_nac VARCHAR(10), direccion VARCHAR(50) , telefono VARCHAR(9), pregunta VARCHAR(100), respuesta VARCHAR(50), id_rol INTEGER, FOREIGN KEY(id_rol) REFERENCES rol(id_rol));';

  //tabla mascota
  tablaMascotas: string =
    'CREATE TABLE IF NOT EXISTS mascotas(id_mascota INTEGER PRIMARY KEY autoincrement, nombre_mascota VARCHAR(20) NOT NULL, genero_mascota VARCHAR(10) NOT NULL, edad_mascota INTEGER NOT NULL, unidad_edad VARCHAR(4) NOT NULL, foto_mascota BLOB, vacunas VARCHAR(2) NOT NULL, detalle_vacuna VARCHAR(250), id_especie INTEGER, FOREIGN KEY(id_especie) REFERENCES especie(id_especie));';

  //tabla adopcion
  tablaAdopcions: string =
    'CREATE TABLE IF NOT EXISTS tadopciones(id_adopcion INTEGER PRIMARY KEY autoincrement, fecha_adopcion VARCHAR(10), estatus BOOLEAN, id_usuario INTEGER, id_mascota INTEGER, FOREIGN KEY(id_usuario) REFERENCES usuarios(id_usuario),FOREIGN KEY(id_mascota) REFERENCES mascota(id_mascota));';

  //variables para insert principales o precargados
  //registro rol
  registroRol: string =
    "INSERT or IGNORE INTO rol(id_rol,nombre_rol) VALUES(1,'administrador'), (2, 'usuario normal');";

  //registro especie
  registroEspecie: string =
    "INSERT or IGNORE INTO especie(id_especie,nombre_especie) VALUES(1,'perro'), (2,'gato'), (3,'conejo');";

  //registro usuario
  registroUsuario: string =
    "INSERT or IGNORE INTO usuarios(id_usuario,nombre_usuario, apellido_usuario, correo, clave, fecha_nac, direccion, id_rol) VALUES(1,'jose','casas', 'admin@helppets.cl', 'Admin.123456', '07/05/1999', 'La Serena 1073', 1);";

  //registro mascota
  registroMascota: string =
    "INSERT or IGNORE INTO mascota(nombre_mascota, edad_mascota, unidad_edad, id_especie) VALUES ('Waton', 2, 'años', 2);";

  //registro adopcion
  registroAdopcion: string =
    "INSERT or IGNORE INTO adopcion(fecha_adopcion, estatus, id_usuario, id_mascota) VALUES ('18/01/2025', 1, (SELECT id_usuario FROM usuarios WHERE nombre_usuario = 'jose' AND apellido_usuario = 'casas'), (SELECT id_mascota FROM mascota WHERE nombre_mascota = 'Waton'));";
  ...
  buscarCuenta(id_usuario: number) {
    ...
    this.database
      .executeSql('SELECT * FROM usuarios WHERE id_usuario = ?', [id_usuario])
      .then((res) => {
        let items: Usuario[] = [];
  ...
  buscarMascotas() {
    ...
    this.database
      .executeSql(
        'SELECT m.id_mascota, m.nombre_mascota, m.genero_mascota, m.edad_mascota, m.unidad_edad, m.foto_mascota, m.vacunas, m.detalle_vacuna, e.nombre_especie FROM mascotas m INNER JOIN especie e ON m.id_especie = e.id_especie',
        []
      )
      .then((res) => {
        // Creo una lista vacía para almacenar los registros del cursor
  ...
  buscarAdopcion() {
    this.database
      .executeSql(
        `SELECT u.nombre_usuario, u.apellido_usuario, u.fecha_nac, u.telefono, u.direccion, a.fecha_adopcion, a.estatus, a.id_adopcion, m.id_mascota, m.nombre_mascota FROM usuarios u INNER JOIN tadopciones a ON u.id_usuario = a.id_usuario INNER JOIN mascotas m ON m.id_mascota = a.id_mascota`,
        []
      )
      .then((res) => {
        let items: Adopcion[] = [];
  ...
  insertarUsuario(
    ...
    this.database
      .executeSql(
        'INSERT INTO usuarios(nombre_usuario, apellido_usuario, correo, clave, pregunta, respuesta, id_rol) VALUES(?, ?, ?, ?, ?, ?, ?)',
        [
          nombre_usuario,
          apellido_usuario,
          correo,
      ...
      .catch((e) => {
        ...
        this.presentAlert(
          'Error',
          'No se pudo registrar el usuario' + JSON.stringify(e)
        );
            ...
            .then(() => {
              ...
              this.database
                ...
                .then(() => {
                  ...
                  this.presentAlert(
                    'Adopción Exitosa',
                    'Tu adopción ha sido registrada.'
                  );
                ...
                .catch((err) => {
                  this.presentAlert(
                    'Error al registrar adopción',
                    'Hubo un problema al registrar tu adopción.'
                  );
                  console.error(err);
                });
            })
  ...
  updateMascota(
    ...
    this.database
      .executeSql('SELECT id_especie FROM especie WHERE nombre_especie = ?', [
        nombreEspecie,
      ])
      .then((res) => {
        if (res.rows.length > 0) {
          ...
          this.database
            .executeSql(
              'UPDATE mascotas SET nombre_mascota =?, genero_mascota =?, edad_mascota =?, unidad_edad =?, foto_mascota =?, vacunas =?, detalle_vacuna =?, id_especie =? WHERE id_mascota =?',
              [
                nombre,
                genero,
                edad,
  ...
  eliminarMascota(id: number) {
    this.database
      .executeSql('DELETE FROM mascotas WHERE id_mascota = ?', [id])
      .then((res) => {
  ...
  updateContra(id_usuario: number, clave: string): Promise<any> {
    return this.database
      .executeSql('UPDATE usuarios SET clave = ? WHERE id_usuario = ?', [
        clave,
        id_usuario,
      ])
      .then((res) => {
  ...
  async seleccionNombre(texto: string) {
    const query = 'SELECT clave FROM usuarios WHERE nombre_usuario = ?';
    try {
      const resutl = await this.database.executeSql(query, [texto]);
      if (resutl.rows.length > 0) {
        return resutl.rows.item(0).clave;
      }
    } catch (e) {
      this.presentAlert(
        'error',
        'no se encontro contraseña' + JSON.stringify(e)
      );
    }
  }

  async seleccionApellido(texto: string) {
    const query = 'SELECT clave FROM usuarios WHERE apellido_usuario = ?';
    try {
      const resutl = await this.database.executeSql(query, [texto]);
      if (resutl.rows.length > 0) {
        return resutl.rows.item(0).clave;
  ...
  async seleccionCorreo(texto: string) {
    const query = 'SELECT clave FROM usuarios WHERE correo = ?';
    try {
      const resutl = await this.database.executeSql(query, [texto]);
      if (resutl.rows.length > 0) {
        return resutl.rows.item(0).clave;
  ...
  recuperarContra(respuesta: string, rama: string) {
    if (rama == 'nombre') {
      return this.seleccionNombre(respuesta);
    } else if (rama == 'apellido') {
      return this.seleccionApellido(respuesta);
    } else {
      return this.seleccionCorreo(respuesta);
    }
  }
}
